...
def _(msg):
    ...
    if msg.text == 'Status':
        bot.reply_to(msg,
                     UI.status.format(
                         ...
                         convert(False), #gpio.get_state(gpio.relay_pin)),
                         convert(verbose),
                         convert(watchdog)
                     ),
                     parse_mode='HTML')
    ...
    if msg.text == 'Turn PC':
        if not helpers.is_pc_up():
            if verbose:
                bot.reply_to(msg, 'Turning On PC {}'.format(ok))
            ...
            return
        if verbose:
            bot.reply_to(msg, 'Turning Off PC {}'.format(ok))
    ...
    if msg.text == 'Turn light':
        if gpio.get_state(gpio.relay_pin4):
            if verbose:
                bot.reply_to(msg, 'Turning Off light {}'.format(ok))
            gpio.hold_off(gpio.relay_pin4)
            return

        if verbose:
            bot.reply_to(msg, 'Turning On light {}'.format(ok))
